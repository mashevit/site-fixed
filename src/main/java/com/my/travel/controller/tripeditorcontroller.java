...
/*		Optional<Dish> studentOptional = dishRepository.findById(id);
	...
	public List<Sightseeing> getsights(int id) {
		return sightseeingRepository.findBytripSightseeingsTripIdtripIn(toArrInt(id));
	}
	
	public List<Integer> toArrInt(int ii){
	...
	public List<Sightseeing> getsights2(Trip id) {
		List<Sightseeing> l2 = sightseeingRepository.findBycityIn(toArrC(id.getCity()));
		return l2;
	}
	
	List<City> toArrC(City c){
		List<City> ans= new ArrayList<>();
		ans.add(c);
		return ans;
	}
	public int getsights3(Trip id) {
		return (cityRepository.findByTripsIn(toArrTrip(id))).getIdcities();
	}
	
	List<Trip> toArrTrip(Trip id){
	...
	public List<Tosave> check1(@RequestParam(value = "id") String id, HttpServletRequest request, HttpServletResponse response, Model model) {
		...
		List<Pic> listp=picRepository.findAllBytripSightseeingIdtripSightseeingIn(intList);
